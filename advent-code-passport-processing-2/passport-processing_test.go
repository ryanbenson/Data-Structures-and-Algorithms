...
func TestProcessExampleInvalid(t *testing.T) {
...
pid:3556412378 byr:2007`
	result := process(passports)
	expected := 0

	if result != expected {
		t.Errorf("When processing passports, looking for invalid passports, got: %v, want: %v.", result, expected)
	}
}

func TestProcessExampleValid(t *testing.T) {
...
iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719`
	...
	if result != expected {
		t.Errorf("When processing passports, looking for valid passports, got: %v, want: %v.", result, expected)
	}
}

func TestProcessExercise(t *testing.T) {
	passports := `eyr:2033
hgt:177cm pid:173cm
