...
func largestRect(island  [][]int) string {
	// go through each row, if you find a 1
	// see if the next item in the row is a 1,
	// or the next item in the column is a 1,
	// if so, run a separate function to find the size
	// keep track of the largest size. is the new rect bigger?
	// if so, replace, and keep going
	return "2x2"
}

func findRectSize(x int, y int) (string, int) {
